...
#changed
from modeling.llama.processing_zero_shot import build_prompt_records_for_llama_truncated
...
def main(cfg):
    ...
    tokenizer = AutoTokenizer.from_pretrained("McGill-NLP/flan-t5-large-weblinx")
    ...
    model = AutoModelForSeq2SeqLM.from_pretrained("McGill-NLP/flan-t5-large-weblinx", **model_kwargs)
    ...
    with torch.cuda.amp.autocast(dtype=torch.float16): #changed to float16
        pbar = tqdm(
