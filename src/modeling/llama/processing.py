...
from .dialogues_process import user_theme
from .pick_sample import get_sample
...
def get_system_prompt_template_for_llama_mc_concise_fewshot():
    sys_prompt_template = (
        "You are an AI assistant with a deep understanding of HTML "
        "and you must predict actions based on a user request, which will be executed. "
        "Use one of the following, replacing [] with an appropriate value: "
...
def get_system_prompt_template_for_llama_mc_concise_history():
    sys_prompt_template = (
        "You are an AI assistant with a deep understanding of HTML "
        "and you must predict actions based on a user request, which will be executed. "
        "Use one of the following, replacing [] with an appropriate value: "
...
def format_all_user_history(similarity_model, utterances_history):
    return user_theme(similarity_model, utterances_history)
...
def build_prompt_records_for_llama_truncated(
    ...
    """
    print("############################ inside build llama prompt ####################")
    if system_prompt_template is None and similarity_model is not None:
        system_prompt_template = get_system_prompt_template_for_llama_mc_concise_history()

    if system_prompt_template is None and similarity_model is None:
        system_prompt_template = get_system_prompt_template_for_llama_mc_concise()
        
    if few_shot:
        # overwrite
        system_prompt_template = get_system_prompt_template_for_llama_mc_concise_fewshot()
    ...
    # with open("/home/kapmcgil/scratch/weblinx/modeling/llama/prompt-debugging/utterance_context.txt", "w") as utt_file:
    #     instructor_chat_turns2 = find_turns_with_instructor_chat(
    #     replay, turn, num_prev_turns=0
    #     )
    ...
    if similarity_model is not None:
        all_chat_turns = find_turns_with_instructor_chat(replay, turn, num_prev_turns=0)
        
        utterances_history = format_utterances_truncated(
            all_chat_turns,
            tokenizer=tokenizer,
            max_tokens=max_utterance_tokens,
            num_utterances=100,
        ...
        all_user_history = format_all_user_history(
        ...
        sys_prompt = system_prompt_template.format(
            ...
            num_prev_turns=num_prev_turns,
            all_user_history=all_user_history,
        )
    
    elif few_shot:
        get_example = get_sample()
        sys_prompt = system_prompt_template.format(
            num_utterances=num_utterances - 1,  # 1 less since we add the first utterance
            utterance_context=utterance_context,
            height=turn.viewport_height,
